...
class MuReNNDirect(torch.nn.Module):
    ...
    """
    def __init__(self, *, J, Q, T, in_channels, J_phi=None, padding_mode="symmetric", scale_factor=1):
        super().__init__()
        if isinstance(Q, int):
    ...
    def forward(self, x):
        ...
        for j in range(self.dtcwt.J):
            xj_real = self.insnorm(lp[j].real)
            xj_imag = self.insnorm(lp[j].imag)
            Wx_j_r = self.conv1d[j](xj_real)
            Wx_j_i = self.conv1d[j](xj_imag)
